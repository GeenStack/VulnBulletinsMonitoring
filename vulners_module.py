...
vulners_api = vulners.VulnersApi(api_key="INSERT YOUR KEY")


def get_bulletins(cve):
    result = {"metadata":{"bulletinFamily":{}}, "bulletins":{}}
    vulners_response = vulners_api.get_multiple_bulletin_references(cve)
    for i in vulners_response:
        for bulletin in vulners_response[i]:
            if bulletin["bulletinFamily"] not in result["metadata"]:
